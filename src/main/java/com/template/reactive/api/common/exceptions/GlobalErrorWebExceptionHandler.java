...
public class GlobalErrorWebExceptionHandler extends AbstractErrorWebExceptionHandler {
	...
	private Map<String, Object> processErrors(Map<String, Object> errors) {
		...
		} else if (error.equalsIgnoreCase("Unsupported Media Type")) {
			String mediaType = StringUtils.substringBetween(trace, "'", "'");
			responseMap.put(STATUS, 415);
			responseMap.put(CODE, "415");
			responseMap.put(MESSAGE, String.format(AppConstants.ERROR_MESSAGE_FORMAT, traceId, "Unsupported Media Type: " + mediaType));
		} else if (trace.contains("ExpiredJwtException")) {
			responseMap.put(STATUS, 403);
			responseMap.put(CODE, "403");
			responseMap.put(MESSAGE, String.format(AppConstants.ERROR_MESSAGE_FORMAT, traceId, "Access token has expired, please log in again."));
		...
		} else if (trace.contains("Request body is missing")) {
			responseMap.put(STATUS, 400);
			responseMap.put(CODE, "400");
		...
		} else if (trace.contains("ServerWebInputException")) {
			String errorDetails = StringUtils.substringBetween(trace, "\"", "\"");
			responseMap.put(STATUS, 400);
			responseMap.put(CODE, "400");
			responseMap.put(MESSAGE, String.format(AppConstants.ERROR_MESSAGE_FORMAT, traceId, errorDetails));
